  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start gap-4">
        <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-surface-2 text-xl font-semibold text-muted">
          {app.name.charAt(0)}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        <Card>
          <CardHeader className="pb-1"><CardTitle className="text-xs text-muted">Top keywords</CardTitle></CardHeader>
          <CardContent className="pt-0 text-xl font-semibold">{topKeywords.length}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1"><CardTitle className="text-xs text-muted">Lost keywords</CardTitle></CardHeader>
          <CardContent className="pt-0 text-xl font-semibold text-red-600">{lostKeywords.length}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1"><CardTitle className="text-xs text-muted">New keywords</CardTitle></CardHeader>
          <CardContent className="pt-0 text-xl font-semibold text-emerald-600">{newKeywords.length}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1"><CardTitle className="text-xs text-muted">Average rank</CardTitle></CardHeader>
          <CardContent className="pt-0 text-xl font-semibold">{avgRank ?? "—"}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1"><CardTitle className="text-xs text-muted">Visibility score</CardTitle></CardHeader>
      ...
      {metadata ? (
        <Card>
          ...
          <CardContent className="grid gap-3 text-sm sm:grid-cols-2">
            <div><span className="text-xs text-muted">Title</span><p>{metadata.title}</p></div>
            <div><span className="text-xs text-muted">Subtitle</span><p>{metadata.subtitle}</p></div>
            <div className="sm:col-span-2"><span className="text-xs text-muted">Keyword field</span><p className="font-mono text-xs">{metadata.keywordsField}</p></div>
            <div className="sm:col-span-2"><span className="text-xs text-muted">Promotional text</span><p>{metadata.promotionalText}</p></div>
          </CardContent>
        </Card>
      ) : (
        <Card>
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
          ...
          <Link href={`/ai-audit?appId=${app.id}`} className="ml-2 text-xs font-medium text-foreground underline">
